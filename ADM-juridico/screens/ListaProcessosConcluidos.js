...
export default function ListaProcessosConcluidos({navigation}) {
  const [processos, setProcessos] = useState([]);
  const [processosID, setProcessosID] = useState([]);
  const [search, setSearch] = useState('');

  useEffect(() => {
    const dbRef = ref(getDatabase(), 'processosConcluidos');
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('TelaVisualizarProcessoConcluido', {
        processoID: item.id,
        advogado: item.advogado,
